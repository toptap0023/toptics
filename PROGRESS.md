...
### 00. (2026-09-05) อัปเกรด UX รอบแรก (จาก code review ยังไม่ได้เห็นจอจริง)

commit `Transaction sheet: tap row to edit...` และ `Categories: show usage counts...`
- แตะแถวรายการเปิดชีตแก้ไขทันที (เดิมต้องแตะขยายก่อนแล้วค่อยกดปุ่มแก้/ลบเล็ก ๆ)
- ปุ่มลบย้ายเข้าไปในชีตแก้ไข เป็นแบบแตะสองครั้ง (armed 3 วิ) แทน window.confirm
- ปุ่ม "บันทึกแล้วเพิ่มต่อ" ตอนเพิ่มรายการ คงประเภทกับวันที่ไว้ ล้างเฉพาะจำนวน/หมวด/โน้ต แล้ว focus ช่องจำนวนให้ (สำหรับ use case บันทึกเป็นรอบเดือนละครั้ง)
- ชีตมี handle bar + slide-up 240ms, backdrop fade, toast fade-in (ปิดเองเมื่อ prefers-reduced-motion)
- ประเภทที่ยังไม่มีหมวด แสดงข้อความพร้อมลิงก์ไปหน้าหมวดหมู่
- หน้าหมวดหมู่: แต่ละหมวดแสดงจำนวนรายการที่ใช้, ปุ่มลบขยายเป็น 40px แตะสองครั้ง พร้อมเตือนว่ารายการจะกลายเป็นไม่มีหมวด, หัวข้อและ subtitle เป็น i18n ครบ

ตรวจในเบราว์เซอร์แล้ว (playwright 390x844, dark + light) ด้วยบัญชีจริงของเจ้าของ: แตะแถวเปิดชีต,
ลบแบบแตะสองครั้ง, บันทึกแล้วเพิ่มต่อ (สร้างรายการ "UX TEST" 1 บาทแล้วลบทิ้ง), หน้าหมวดหมู่ ทำงานครบ
มี test user ค้างที่ยังไม่ยืนยันอีเมล `10xiconkaset.ai+uxtest@gmail.com` ใน Supabase auth ลบได้จาก dashboard
